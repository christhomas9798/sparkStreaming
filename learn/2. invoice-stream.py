...
class invoiceStream():
    ...
    def appendInvoices(self,flattened_df):
        return flattened_df.writeStream.format('delta').option('checkpointLocation','/FileStore/spark_streaming/json/checkpoint/invoice').outputMode('append').trigger(processingTime='20 seconds').toTable('invoice_line_items')
